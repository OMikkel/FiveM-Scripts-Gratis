...
vRPom = {}
Tunnel.bindInterface("t1ger_drugs",vRPom)
...
function OpenDrugMenu()
	...
	for k,v in pairs(Config.ListOfDrugs) do
		if v.Enabled == true then
			if v.drug == "coke" then
				cokePrice = v.BuyPrice
				cokeName = v.label
			end
			if v.drug == "meth" then
				methPrice = v.BuyPrice
				methName = v.label
			end
			if v.drug == "weed" then
				weedPrice = v.BuyPrice
				weedName = v.label
			end
...
RegisterNUICallback("exit", function(data, cb)
	local ped = GetPlayerPed(-1)
	CloseDrugMenu()
	TriggerEvent("pNotify:SendNotification",{text = {"You cancelled the request"}, type = "error", queue = "global",timeout = 4000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
	ClearPedTasks(ped)
	FreezeEntityPosition(ped,false)
end)

RegisterNUICallback("drugs1", function(data, cb)
	local ped = GetPlayerPed(-1)
	CloseDrugMenu()
	TriggerServerEvent("t1ger_drugs:GetSelectedJob",data.item, data.price, 1, 3)
	ClearPedTasks(ped)
	FreezeEntityPosition(ped,false)
end)
RegisterNUICallback("drugs2", function(data, cb)
	local ped = GetPlayerPed(-1)
	CloseDrugMenu()
	TriggerServerEvent("t1ger_drugs:GetSelectedJob",data.item, data.price, 2, 6)
	ClearPedTasks(ped)
	FreezeEntityPosition(ped,false)
end)
RegisterNUICallback("drugs3", function(data, cb)
	local ped = GetPlayerPed(-1)
	CloseDrugMenu()
	TriggerServerEvent("t1ger_drugs:GetSelectedJob",data.item, data.price, 4, 10 )
	ClearPedTasks(ped)
	FreezeEntityPosition(ped,false)
end)
...
AddEventHandler("t1ger_drugs:BrowseAvailableJobs",function(spot,drugType,minReward,maxReward)
	...
	if currentID == 100 then
		TriggerEvent("pNotify:SendNotification",{text = {"No jobs are currently available, please try again later!"}, type = "error", queue = "global",timeout = 4000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
	else
		SelectedID = id
	...
	while not JobCompleted and not StopTheJob do
		...
		if Config.Jobs[id].InProgress == true then
			...
			if (GetDistanceBetweenCoords(coords, selectedJob.Spot.x, selectedJob.Spot.y, selectedJob.Spot.z, true) < 150) and not selectedJob.VanSpawned then
				...
				local mhash = GetHashKey(Config.JobVan)
				...
				if HasModelLoaded(mhash) then
					local x,y,z = table.unpack(jobCoords)
					local nveh = CreateVehicle(mhash, x,y,z+0.5, selectedJob.Heading, true, false) -- added player heading
					
					SetEntityCoordsNoOffset(nveh, selectedJob.Spot.x, selectedJob.Spot.y, selectedJob.Spot.z)
					SetEntityHeading(nveh,selectedJob.Heading)
			...
			if NearOtherVehicle == true and drugBoxInHand and IsControlJustPressed(0, 38) then
				TriggerEvent("pNotify:SendNotification",{text = "Job completed",type = "success",timeout = 3000,layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},})
				PlaySoundFrontend(-1, "Mission_Pass_Notify", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 0)
		...
		repeat
			...
			if distance < 2 and CanSellToPed(ped) and not IsPedInAnyVehicle(player, true) then
				if Config.Enable3DTextToSell then
					...
					TriggerEvent("pNotify:SendNotification",{text = "Press H to offer drugs",type = "info",timeout = (2000),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})  
				end
				if IsControlJustPressed(1,74) then
					...
					if chance == 1 or chance == 2 then
						...
						if chance == 1 then
							...
							TriggerEvent("pNotify:SendNotification",{text = "Your offer was rejected and Police is notified",type = "error",timeout = (2000),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})  
						else
							TriggerEvent("pNotify:SendNotification",{text = "Your offer was rejected",type = "error",timeout = (2000),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})  
